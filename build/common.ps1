    ...
    if (-not $SkipExecution) {
        ...
        try {
            ...
            elseif (-not $err) {
                ...
                Error-Log "[FAILED +$(Format-ElapsedTime $sw.Elapsed)] $BuildStep. Err: $err"
            }
        }
    }
    else {
...
Function Install-DotnetCLI {
    ...
    ForEach ($CliBranch in $CliBranchList) {
        ...
        if ($Force -or -not (Test-Path $probeDotnetPath)) {
            Trace-Log "$DotNetInstall -Channel $($cli.Channel) -InstallDir $($cli.Root) -Version $($cli.Version) -Architecture $arch -NoPath -Verbose"
            & $DotNetInstall -Channel $cli.Channel -InstallDir $cli.Root -Version $cli.Version -Architecture $arch -NoPath -Verbose -AzureFeed "https://dotnetcli.azureedge.net/dotnet"
        }
